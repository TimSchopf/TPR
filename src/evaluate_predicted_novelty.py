...
predictor_llm_name = "gpt-oss-120b"
approach = "novelty_checker"
with open('../data/json/' + approach + '_novelty_predictions-' + predictor_llm_name + '.json', 'r') as f:
...
eval_metrics = asyncio.run(eval_bench(
    ...
    predicted_reasonings=[p['Review'] for p in predictions],
    gold_reasonings=ds['test']['novelty_reasoning'],
    predicted_scores=[p['Class'] for p in predictions],
    gold_scores=ds['test']['novelty_score'],
    n_parallel=10,
    temperature=0,
    max_retries=100,
    retry_backoff=1.0,
    novelty_score_metrics_only=True
))

# Save to disk
save_path = '../data/json/' + approach + '_novelty_pred_evaluation-' + predictor_llm_name + '.json'
