...
# evaluate the traine model
def format_example_no_tokenizer(example):
    
    system_prompt = "You are an expert researcher experienced in judging the novelty of a research idea."
    user_prompt = build_judgment_prompt(
            research_idea=example['research_idea'],
            related_works=example['related_works'],
            class_descriptions=label_descriptions
    )
...
# init pipeline with fine-tuned model
pipe = pipeline("text-generation", model=trainer.model, tokenizer=trainer.processing_class)

# prepare test input data
test = ds["test"].map(format_example_no_tokenizer)

# Generate responses with fine-tuned model for all test data
llm_responses = []
for output in tqdm(pipe(
    KeyDataset(test, "messages"),  # Iterates over 'messages' column
    max_new_tokens=5000,
    do_sample=True,
    batch_size=8,  # Adjust based on your GPU memory
    truncation=False
),total=test.num_rows):
    llm_responses.append(output[0]['generated_text'][-1]['content'])  # Last message is the assistant's
...
def extract_prediction_value(text: str, approach: str) -> int:
    ...
    """
    if approach == "novelty_checker":
        # Regex pattern:
        # - optional ** around Class
        # - single or double quotes around Class
        # - optional spaces around colon
        # - captures integer value
        pattern = r'(?i)(?:\*\*)?\s*["\']?Class["\']?\s*(?:\*\*)?\s*[:=]\s*["\']?(\d+)'

    elif approach == "ai_researcher" or "naive_prompting" or "naive_prompting_low_reasoning":
        ...
        pattern = r'(?:\*\*)?\s*(?:["\']?)Rating(?: \(1[\-\u2010\u2011\u2012\u2013\u2014]5\))?(?:["\']?)\s*(?:\*\*)?\s*:\s*(\d+)'
...
textual_predictions = []
for response in llm_resonses:
    textual_predictions.append(extract_prediction_value(response, "naive_prompting"))

print("Number of textual predictions:", len(textual_predictions))


y_test = np.array(ds["test"]["novelty_score"])
textual_predictions = np.array(textual_predictions)

# -----------------------------------------
# F1 scores of textual LLM predictions
# -----------------------------------------
