...
available_gb = memory.available / (1024 ** 3)  # Convert bytes to GB
print(f"Available CPU RAM: {available_gb:.2f} GB ({memory.available * 100 / memory.total:.1f}% available)")
...
model = AutoModelForCausalLM.from_pretrained(model_id, cache_dir=cache_dir, trust_remote_code=True, dtype=torch.bfloat16, quantization_config=Mxfp4Config(dequantize=True), low_cpu_mem_usage=True, use_safetensors=True, offload_folder="./offload_dir", offload_buffers=False, use_cache=False, device_map="auto")
print("loaded model:", model.config._name_or_path)

# check RAM usage after model load
process = psutil.Process(os.getpid())
print(f"RAM usage: {process.memory_info().rss / 1024**3:.2f} GB")  # Poll during load
...
# If using a Llama-like architecture (which GPT-OSS is), ensure this specific 
# flag is set to handle the meta-device gradient mismatch:
...
training_args = TrainingArguments(
    ...
    gradient_checkpointing_kwargs={"use_reentrant": True}, # Setting this to false fixes the "different number of tensors" error for fsdp training.
    num_train_epochs=2,
    ...
    #    "fsdp_cpu_ram_efficient_loading": True, # CRITICAL: Loads shards one by one
    #    "fsdp_sync_module_states": True,        # Syncs weights from rank 0 to others
    #    "fsdp_offload_params": True,            # Helps if VRAM is still tight on 40GB cards
    #},
...
trainer = Trainer(
    ...
    eval_dataset=test_dataset,#.select(range(n)),
