...
def llm_inference(model, tokenizer, messages, max_new_tokens=5000, temperature=1.0):

    while True:   # <--- repeat generation until conditions are met
        # -------------------------
        # 1) tokenize
        # -------------------------
        ...
        # -------------------------
        # 4) extract tokens and decode
        # -------------------------
        prompt_len = inputs["input_ids"].shape[-1]
        parsed_outputs = "".join(
            tokenizer.batch_decode(outputs.sequences[0][prompt_len:])
        )
        #print(parsed_outputs)
        # -------------------------
        # --- Condition check -----
        # -------------------------
        if "<REVIEW>" in parsed_outputs:
            idx = parsed_outputs.index("<REVIEW>")
            if idx >= 100:
                # both conditions satisfied → break loop and return
                break
    ...
    # --------------------------------------------------------
    # If we reached here, parsed_outputs is valid.
    # Now extract hidden states before returning.
    # --------------------------------------------------------

    inputs.to('cpu')
    last_layer_states = []
    for timestep in outputs.hidden_states:
        last_layer_states.append(timestep[-1].cpu())
...
messages = built_novelty_nle_prompt(
            ...
            similar_documents=ds['train']['related_works'][i][:15],
            few_shot_examples=few_shot_examples,
            class_descriptions=label_descriptions
        )
print(len(ds['train']['related_works'][i]))

parsed_outputs, last_layer_states = llm_inference(model=model,
            tokenizer=tokenizer,
            messages=messages,
            max_new_tokens=5000,
            temperature=1.0
...
def run_split(split_name, data_split, approach_name):
    ...
    for i in tqdm(missing_indices, desc=f"Predict {split_name}"):
        ...
        messages = built_novelty_nle_prompt(
            ...
            similar_documents=data_split['related_works'][i][:15],
            few_shot_examples=few_shot_examples,
            class_descriptions=label_descriptions
        )
...
#                     RUN
# -----------------------------------------------------
run_split("train", ds["train"],"nle_only_r3")
run_split("test", ds["test"], "nle_only_r3")
