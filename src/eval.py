...
from research_novelty_judgment.src.RINoBench_preprocessing.pydantic_models import *
from research_novelty_judgment.src.RINoBench_preprocessing.prompts import *
...
async def compute_justification_metrics(client: LLMClient, llm_name: str, research_idea: dict, related_works: list[dict], predicted_reasoning: str, gold_reasoning: str, temperature: float | None = 0, max_retries: int = 3, retry_backoff: float = 1.0) -> dict:
...
async def eval_bench(client: LLMClient, llm_name: str, research_ideas: list[dict], related_works: list[list[dict]], predicted_reasonings: list[str], gold_reasonings: list[str], predicted_scores: list[int], gold_scores: list[int], n_parallel: int = 5, temperature: float | None = 0, max_retries: int = 3, retry_backoff: float = 1.0, novelty_score_metrics_only: bool = False):
    """
    Calculates numeric evaluation metrics and subsequently runs compute_eval_metrics() concurrently across multiple examples, limited to `n_parallel` simultaneous tasks, to compute additional justification evaluation metrics.
    ...
    if not novelty_score_metrics_only:
        ...
        async def sem_task(idx):
            async with semaphore:
                result = await compute_justification_metrics(
                    client=client,
                    llm_name=llm_name,
