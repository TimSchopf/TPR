...
messages = built_novelty_nle_prompt(
            ...
            similar_documents=ds['test']['related_works'][i][:20], # use only first n related works to avoid OOM error with huge prompts
...
def run_split(split_name, data_split, approach_name):
    ...
    for i in tqdm(missing_indices, desc=f"Predict {split_name}"):
        ...
        messages = built_novelty_nle_prompt(
            ...
            similar_documents=data_split['related_works'][i][:30], # use only first n related works to avoid OOM error with huge prompts
            few_shot_examples=few_shot_examples,
            class_descriptions=label_descriptions
        )
...
#                     RUN
# -----------------------------------------------------
run_split("train", ds["train"],"nle_only_r3"+"_"+reasoning_effort)
run_split("test", ds["test"], "nle_only_r3"+"_"+reasoning_effort)